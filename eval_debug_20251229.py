...
def plot_multiple_results(result_files, labels, save_dir, test_id):
    ...
    for idx, rf in enumerate(result_files):
        ...
        plt.plot(fpr, tpr, lw=2, linestyle=linestyles[idx % len(linestyles)], label=f'{label} (AUC={roc_auc:.3f})', color=colors[idx % len(colors)])
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"roc_compare_{test_id}.png"), dpi=300)
    plt.close()
