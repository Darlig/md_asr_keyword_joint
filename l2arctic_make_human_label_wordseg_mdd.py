...
def make_human_label_json(source_annot_dir: str, target_json_path: str, phone_list: List[str]):
    ...
    for root, dirs, files in os.walk(source_annot_dir):
        ...
        for file in files:
            if file.endswith('.TextGrid'):
                ...
                if not annot_dict.get('if_invalid', False):
    ...
    print(f"first 20 unk phones: {unk_phn_list[:20]}")
...
def utt_annot_to_label(utt_annot_path: str, phone_list: List[str]) -> Dict[str, any]:
    ...
    for phone_interval in non_silence_phone_intervals:
        ...
        if ',' in phn:
            canonical_phn, actual_phn, error_type = re.sub(r'\s+', '', phn).split(',')
            if error_type not in ['s', 'd']:
                #print(f"Error type {error_type} not supported, only support s")
                return None

            canonical_phn = re.sub(r'[`)]', '', canonical_phn).upper()
            actual_phn = re.sub(r'[\*]', '', actual_phn)
            if actual_phn == 'sil':
                actual_phn = 'spn'
            if canonical_phn not in phone_list:
                if canonical_phn not in unk_phns:
                    unk_phns.append(canonical_phn)
                return {'if_invalid': True, 'unk_phns': unk_phns}

            #if actual_phn not in phone_list:
            if actual_phn not in phone_list and actual_phn != 'spn':
                if actual_phn not in unk_phns:
                    unk_phns.append(actual_phn)
                return {'if_invalid': True, 'unk_phns': unk_phns}
            phones.append(canonical_phn)
            phones_actual.append(actual_phn)
    ...
    for word_interval in non_empty_word_intervals:
        ...
        while phone_idx < len(non_silence_phone_intervals):
            ...
            if phone_start >= word_end:
                ...
                word_phones_actual.append(phones_actual[phone_idx])
        ...
        if not word_phones and phone_idx < len(non_silence_phone_intervals):
            ...
            if min_distance < 0.1:  # 100ms阈值
                ...
                word_phones_actual.append(phones_actual[closest_phone_idx])
        ...
        word_segments.append({
            'text': word_text,
            'phones': word_phones,
            'phones_actual': word_phones_actual,
