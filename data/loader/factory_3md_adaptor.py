...
def process_sampled_keyword_from_label(
        data: Iterator[Dict], positive_prob: float=0.5, neg_len: int = None, special_token: Dict = {}, max_keyword_len: int=8, phonetic_auxiliary: Dict = {}, phone_data_aug: str = None, p_tone: float = 0.0
):
    ...
    for sample in data:
        ...
        kw, new_phn_label, new_bpe_label, kw_pos, md_label = utils.inject_special_token(
            ...
            phonetic_auxiliary=phonetic_auxiliary, phone_data_aug=phone_data_aug, p_tone=p_tone
        )
...
def process_sampled_keyword_from_label_md(
    ...
    for sample in data:
        ...
        # === Multi-annotator MD labels ===
        # Recommended json keys: md_label1 / md_label2 / md_label3
        # Backward compatible: if only md_label exists, reuse it for all 3 heads.
        if ('md_label1' in sample) and ('md_label2' in sample) and ('md_label3' in sample):
            md_label1 = copy.deepcopy(sample['md_label1'])
            md_label2 = copy.deepcopy(sample['md_label2'])
            md_label3 = copy.deepcopy(sample['md_label3'])
        else:
            md_label = copy.deepcopy(sample['md_label'])
            md_label1 = copy.deepcopy(md_label)
            md_label2 = copy.deepcopy(md_label)
            md_label3 = copy.deepcopy(md_label)
        ...
        # NOTE: make_keyword_md currently does not modify md_label; it mainly decides pos/neg.
        kw, kw_pos, kw_length, pos, target, _ = utils.make_keyword_md(
            candidate_seq=new_phn_label, negative_seq=sample['neg_candidate'], md_label=md_label1,
            positive_prob=positive_prob, neg_len=neg_len, kw_position_candidate=sample['kw_candidate'],
            corrupt_label=corrupt_label, max_keyword_len=max_keyword_len
        )

        # IMPORTANT: inject_special_token_md contains random augmentation logic.
        # We must apply the *same* random decisions to all 3 annotators' md_label.
        #_rng_state = random.getstate()
        kw, new_sph_label, new_bpe_label, kw_pos, _ = utils.inject_special_token_md(
            keyword=kw, keyword_length=kw_length, positive=pos, label=new_sph_label,
            keyword_pos=kw_pos, special_token=special_token,  bpe_label=new_bpe_label, bpe_candidate=bpe_candidate,
            phonetic_auxiliary=phonetic_auxiliary, md_label=md_label1, phone_data_aug=phone_data_aug
        )
        #random.setstate(_rng_state)
        #_, _, _, _, md_label2 = utils.inject_special_token_md(
        #    keyword=kw, keyword_length=kw_length, positive=pos, label=new_sph_label,
        #    keyword_pos=kw_pos, special_token=special_token,  bpe_label=new_bpe_label, bpe_candidate=bpe_candidate,
        #    phonetic_auxiliary=phonetic_auxiliary, md_label=md_label2, phone_data_aug=phone_data_aug
        #)
        #random.setstate(_rng_state)
        #_, _, _, _, md_label3 = utils.inject_special_token_md(
        #    keyword=kw, keyword_length=kw_length, positive=pos, label=new_sph_label,
        #    keyword_pos=kw_pos, special_token=special_token,  bpe_label=new_bpe_label, bpe_candidate=bpe_candidate,
        #    phonetic_auxiliary=phonetic_auxiliary, md_label=md_label3, phone_data_aug=phone_data_aug
        #)

        # Keep md_label as an alias of md_label1 for backward compatibility (md_label_len is derived from it).
        sample.update({
            'keyword': kw,
            'phn_label': new_sph_label,
            'bpe_label': new_bpe_label,
            'target': target,
            'md_label': md_label1,
            'md_label1': md_label1,
            'md_label2': md_label2,
