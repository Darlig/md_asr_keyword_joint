...
for uttid, utt_item in score_item.items():
    for i, word in enumerate(utt_item['words']):
        ...
        for j in range(0, len(phones), 2):
            ...
            if phones_actual[j] == 'y':
                phones_actual[j] = 'ii'
            if phones_actual[j] == 'none' and phones_accuracy[j] == 1:
                phones_actual[j] = phones[j]
            #if phones_actual[j] == 'none' and phones_actual[j+1].startswith('iong') and phones[j] == 'oo' and phones[j+1].startswith('ou'):
            #    phones_actual[j] = 'oo'
            #if phones[j] != '':
