...
class Trainer():
    ...
    def load_ckpt_freeze(self, ckpt):
        ...
        if any(k.startswith("module.") for k in model.keys()):
            model = {k[len("module."):]: v for k, v in model.items()}

        # 推断当前模型有几个 head（默认 3；若你未来改成更多，也自动适配）
        #heads = set()
        #for k in cur_sd.keys():
        #    m = re.match(r"det_nets\.(\d+)\.", k)
        #    if m:
        #        heads.add(int(m.group(1)))
        #n_heads = (max(heads) + 1) if heads else 3
        n_heads = 3

        new_model = {}
        # 先拷贝除 det_net.* 之外的所有参数
        for k, v in model.items():
            if not k.startswith("det_net."):
                new_model[k] = v

        # 再把 det_net.* 复制成 det_nets.{h}.*（三份/多份）
        for k, v in model.items():
            if k.startswith("det_net."):
                suffix = k[len("det_net."):]  # e.g. "0.w1.weight"
                for h in range(n_heads):
                    new_k = f"det_nets.{h}.{suffix}"  # -> "det_nets.0.0.w1.weight" ...
                    new_model[new_k] = v

        model = new_model
        self.recorder.info(f"[load_ckpt_freeze] Expanded det_net.* -> det_nets.{{0..{n_heads-1}}}.* for init")
        ...
        for name, param in self.model.named_parameters():
            if (name.startswith("hubert_")) or (name.startswith("au_")) or  ( name.startswith("phn_asr_crit")):
                param.requires_grad = False
        for name, param in self.model.named_parameters():
    ...
    def avg_model(self):
        # average the last N model (default 11)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            if self.rank == 0 and epoch % 5 == 0:
                ...
                self.recorder.info("det_nets.1.1.weight: {}".format(
                    self.model.get_parameter('det_nets.1.1.weight')
                ))
