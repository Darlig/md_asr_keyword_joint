...
# If not None, evaluate metrics at these fixed score thresholds
given_thresholds = [0.3158619999885559]  # e.g., [0.3, 0.5, 0.7]
#given_thresholds = [0.23421068489551544]  # e.g., [0.3, 0.5, 0.7]
...
# Helper to compute metrics at a fixed threshold
def _metrics_at_threshold(y_true, y_scores, thr: float):
    """Compute metrics at a fixed score threshold.

    Returns: (precision, recall, f1, predicted_positive_rate, TP, FP, TN, FN)
    """
    # accept either torch tensors or numpy arrays
    if hasattr(y_scores, 'numpy'):
        y_scores = y_scores.numpy()
    ...
    predicted_positive_rate = float(np.mean(pred_pos)) if len(pred_pos) else 0.0
    return float(precision), float(recall), float(f1), float(predicted_positive_rate), TP, FP, TN, FN
...
def plot_result(hyp, ground_truth, save_dir, test_id):
    ...
    # Given-threshold evaluation (optional)
    if given_thresholds is not None:
        for thr in given_thresholds:
            p_thr, r_thr, f1_thr, ppr_thr, TP, FP, TN, FN = _metrics_at_threshold(ground_truth_np, hyp_np, thr)
            print(f"@threshold={thr:.6f}: precision={p_thr:.4f}, recall={r_thr:.4f}, f1_score={f1_thr:.4f}, auc_roc={roc_auc:.4f}, predicted_positive_rate={ppr_thr:.4f} (TP={TP}, FP={FP}, TN={TN}, FN={FN})")
    ...
    with open(os.path.join(save_dir, f"roc_pr_{test_id}.txt"), 'w') as f_roc:
        ...
        #         p_thr, r_thr, f1_thr, ppr_thr, TP, FP, TN, FN = _metrics_at_threshold(ground_truth_np, hyp_np, thr)
        #         f_roc.write(
        #             f"@threshold={thr:.6f}: precision={p_thr:.4f}, recall={r_thr:.4f}, f1_score={f1_thr:.4f}, auc_roc={roc_auc:.4f}, predicted_positive_rate={ppr_thr:.4f} (TP={TP}, FP={FP}, TN={TN}, FN={FN})\n"
        #         )
