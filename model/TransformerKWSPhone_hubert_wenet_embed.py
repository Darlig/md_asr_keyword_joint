...
class TransformerKWSPhone_hubert_wenet_embed(nn.Module):
    def __init__(
        self,
        audio_net_config,
        kw_net_config,
        ...
        **kwargs,
        ...
        super(TransformerKWSPhone_hubert_wenet_embed, self).__init__()
        self.sok = sok
        self.eok = eok
    ...
    def evaluate(self, input_data):
        ...
        #print(f"evaluate() input sph size: {sph_input.shape}")
        sph_mask = ~NM.make_mask(sph_len).unsqueeze(1)
        kw_mask = ~NM.make_mask(kw_len).unsqueeze(1)

        cross_mask = ~NM.combine_mask(sph_mask.squeeze(1), kw_mask.squeeze(1), 1)
        sph_emb = self.hubert_trans(sph_input)
        #print(f"sph_emb size: {sph_emb.shape}")
