...
def save_results(hyp, ground_truth, ids, save_dir, test_id):
    ...
    if ids_np is None:
        ...
        np.savez_compressed(os.path.join(save_dir, f"results_{test_id}.npz"), hyp=hyp_np, gd=ground_np, ids=ids_np)
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        if ids_np is None:
            ...
            writer.writerow(['id', 'hyp', 'ground_truth'])
            for _id, h, g in zip(ids_np.tolist(), hyp_np.tolist(), ground_np.tolist()):
                writer.writerow([_id, h, int(g)])
...
def run(config, ckpt, data_list_file, save_dir, test_id, num_add, load_results_path=None, load_if_exists=False):
    ...
    for i, one_test_obj in enumerate(tr_list):
        ...
        for i in range(len(aug_keywords)):
            ...
            det_result = apply_temperature_to_prob(det_result, temperature)
...
if __name__ == '__main__':
    ...
    if args.temp_file is not None:
        ...
        temperature = float(tmp["temperature"])
