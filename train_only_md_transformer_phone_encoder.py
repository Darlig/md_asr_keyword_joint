...
class Trainer():
    ...
    def load_ckpt_freeze(self, ckpt):
        ...
        for name, param in self.model.named_parameters():
            if ( not name.startswith("phn_emb")) and ( not name.startswith("kw_")) and ( not name.startswith("det_net")) and ( not name.startswith("md_transformer")) :
                param.requires_grad = False
        for name, param in self.model.named_parameters():
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            if self.rank == 0 and epoch % 5 == 0:
                ...
                self.recorder.info("phn_emb.emb.weight: {}".format(
                    self.model.get_parameter('phn_emb.emb.weight')
                ))
                ...
                self.recorder.info("md_transformer.3.cross_att.v.0.weight: {}".format(
                    self.model.get_parameter('md_transformer.3.cross_att.v.0.weight')
                ))
