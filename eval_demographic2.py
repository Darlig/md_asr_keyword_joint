...
def _recall_at_precision(precision, recall, p, mode="max", thresholds=None):
    """Return (best_recall, matched_precision, f1, matched_threshold) at/above target precision p.

    - mode="max": 在 precision >= p 的点集中，找 recall 最大的点；若有并列，取 precision 最小者。
    - mode="interp": 在按 precision 升序插值得到 r=recall(p)，返回 (r, p, f1(p,r), matched_threshold)。

    Notes (sklearn.precision_recall_curve):
        len(precision) == len(recall) == len(thresholds) + 1
        thresholds 对应 precision[1:], recall[1:]
        因此 best_idx==0 时没有有效阈值，matched_threshold 设为 +inf。

    If thresholds is None, matched_threshold will be returned as None.
    """
    p = float(np.clip(p, 0.0, 1.0))
    precision = np.asarray(precision, dtype=float)
    recall = np.asarray(recall, dtype=float)

    def _thr_from_idx(best_idx: int):
        ...
        if best_idx <= 0:
            return float('inf')
        thr_idx = best_idx - 1
        thr_idx = min(max(thr_idx, 0), len(thr) - 1)
    ...
    if mode == "interp":
        ...
        r_val = float(np.interp(p, prec_sorted, rec_sorted))
        f1 = (2 * p * r_val) / (p + r_val) if (p + r_val) > 0.0 else 0.0
        # For interpolated point, threshold is not uniquely defined; use nearest original point.
        best_idx = int(np.argmin(np.abs(precision - p)))
        t = _thr_from_idx(best_idx)
        return r_val, float(p), float(f1), t

    # mode == "max"
    mask = (precision >= p)
    if not np.any(mask):
        return None, None, None, None  # 达不到目标 precision
    ...
    best_idx = int(tie_idxs[np.argmin(precision[tie_idxs])])
    ...
    t = _thr_from_idx(best_idx)
    return best_r, best_p, float(f1), t

def _precision_at_recall(precision, recall, r, mode="max", thresholds=None):
    """Return (best_precision, matched_recall, f1, matched_threshold) at/above target recall r.

    - mode="max": 在 recall >= r 的点集中，找 precision 最大的点；若有并列，取 recall 最大者。
    - mode="interp": 在按 recall 升序插值得到 p=precision(r)，返回 (p, r, f1(p,r), matched_threshold)。
    ...
    if mode == "interp":
        ...
        rec_sorted = recall[order]
        prec_sorted = precision[order]
        p_val = float(np.interp(r, rec_sorted, prec_sorted))
        f1 = (2 * p_val * r) / (p_val + r) if (p_val + r) > 0.0 else 0.0
        best_idx = int(np.argmin(np.abs(recall - r)))
        t = _thr_from_idx(best_idx)
        return float(p_val), float(r), float(f1), t

    # mode == "max"
    mask = (recall >= r)
    if not np.any(mask):
        return None, None, None, None  # 达不到目标 recall
    ...
    best_idx = int(tie_idxs[np.argmax(recall[tie_idxs])])

    best_p = float(precision[best_idx])
    best_r = float(recall[best_idx])
    f1 = (2 * best_p * best_r) / (best_p + best_r) if (best_p + best_r) > 0.0 else 0.0
    t = _thr_from_idx(best_idx)
    return best_p, best_r, float(f1), t
    ...
    with open(os.path.join(save_dir, f"roc_pr_{test_id}.txt"), 'w') as f_roc:
        ...
        if target_recalls is not None:
            for r in target_recalls:
                matched_p, matched_r, matched_f1, matched_thr = _precision_at_recall(precision, recall, r, mode=mode, thresholds=pr_thresholds)
                if matched_p is not None:
                    print(f"@recall={r:.4f}: precision={matched_p:.4f}, recall={matched_r:.4f}, f1_score={matched_f1:.4f}, thr={matched_thr} ({mode})")
                    f_roc.write(f"@recall={r:.4f}: precision={matched_p:.4f}, recall={matched_r:.4f}, f1_score={matched_f1:.4f}, thr={matched_thr} ({mode})\n")
                else:
        ...
        if target_precisions is not None:
            for p in target_precisions:
                matched_r, matched_p, matched_f1, matched_thr = _recall_at_precision(precision, recall, p, mode=mode, thresholds=pr_thresholds)
                if matched_r is not None:
                    print(f"@precision={p:.4f}: precision={matched_p:.4f}, recall={matched_r:.4f}, f1_score={matched_f1:.4f}, thr={matched_thr} ({mode})")
                    f_roc.write(f"@precision={p:.4f}: precision={matched_p:.4f}, recall={matched_r:.4f}, f1_score={matched_f1:.4f}, thr={matched_thr} ({mode})\n")
                else:
                    print(f"recall@precision={p:.4f} ({mode}): N/A (recall 未达到)")
                    f_roc.write(f"recall@precision={p:.4f} ({mode}): N/A (recall 未达到)\n")
