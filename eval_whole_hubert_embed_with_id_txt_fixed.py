...
def save_results(hyp, gd, ids, save_dir, test_id):
    """Save hyp and ground truth arrays to compressed npz and csv, and also a txt with id column.

    - CSV: unchanged columns ['hyp', 'ground_truth']
    - TXT: columns ['id', 'hyp', 'ground_truth'], where id = '{uttid}.{phn_index}'
    """
    hyp_np = hyp.numpy() if hasattr(hyp, 'numpy') else np.asarray(hyp)
    gd_np = gd.numpy() if hasattr(gd, 'numpy') else np.asarray(gd)

    os.makedirs(save_dir, exist_ok=True)

    # npz (keeps the original keys hyp/gd)
    np.savez_compressed(os.path.join(save_dir, f"results_{test_id}.npz"), hyp=hyp_np, gd=gd_np)
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        for h, g in zip(hyp_np.tolist(), gd_np.tolist()):
            writer.writerow([h, int(g)])

    # txt (new, with id column)
    txt_path = os.path.join(save_dir, f"results_{test_id}.txt")
    if len(ids) != len(hyp_np):
        raise ValueError(f"Length mismatch: len(ids)={len(ids)} vs len(hyp)={len(hyp_np)}. "
                         f"Make sure you generate one id per phone score.")
    with open(txt_path, 'w') as f:
        f.write("id,hyp,ground_truth\n")
        for i, (h, g) in enumerate(zip(hyp_np.tolist(), gd_np.tolist())):
            f.write(f"{ids[i]},{h},{int(g)}\n")
...
def run(config, ckpt, data_list_file, save_dir, test_id, num_add, load_results_path=None, load_if_exists=False):
    ...
    for i, one_test_obj in enumerate(tr_list):
        ...
        for i in range(len(aug_keywords)):
            ...
            ids.extend([f"{uttid}.{k}" for k in range(det_result.numel())])
