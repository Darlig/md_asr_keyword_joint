...
def build_canonical_and_actual_from_py_pybz(py_pair, pybz_tok: str):
    """
    Build canonical/actual phones aligned by token.

    - Canonical output is based on PYBZ canonical (normalized and adjusted by the existing rules).
    - PY canonical is still parsed/normalized for mismatch statistics and optional reference output.
    - Actual output follows PYBZ parentheses rule (then normalized):
        * If a part has no parentheses => treat as correct => actual piece equals PYBZ canonical piece.
        * If parentheses exist => actual piece is the inside text (or '*').

    Returns: (canon_pybz_pair, canon_py_pair, actual_pair)
    """
    ...
    # Derived PYBZ canonical pieces for default actual (when no parentheses)
    bz_final_c, bz_tone_c = _split_final_tone_keep_empty(_clean(bz_finaltone2).lower())
    bz_final_c = _clean(bz_final_c).lower()
    bz_tone_c = _strip_dash_markers(_clean(bz_tone_c))
    ...
    # If no parentheses => correct => use PYBZ canonical piece
    if init_has:
    ...
    # ensure exact match uses PYBZ canonical string
    if actual[0] != "*" and actual[0] == canon_pybz[0]:
        actual[0] = canon_pybz[0]
    if actual[1] != "*" and actual[1] == canon_pybz[1]:
        actual[1] = canon_pybz[1]

    return canon_pybz, canon, actual
...
def build_entry_for_textgrid(path: Path, hz_tier="HZ", py_tier="PY", pybz_tier="PYBZ"):
    ...
    for i in range(n):
        ...
        canon_phones = []          # canonical (PYBZ)
        canon_phones_py = []       # canonical (PY) for reference
        actual_phones = []
        ...
        for py_pair, pybz_tok in zip_longest(py_pairs, pybz_toks, fillvalue=None):
            if py_pair is None:
                ...
                if pybz_tok is None:
                    canon_pair_pybz = ["", ""]
                    canon_pair_py = [py_pair[0], py_pair[1]]
                    actual_pair = ["", ""]
                else:
                    canon_pair_pybz, canon_pair_py, actual_pair = build_canonical_and_actual_from_py_pybz(py_pair, pybz_tok)

            canon_phones.extend(canon_pair_pybz)
            canon_phones_py.extend(canon_pair_py)
        ...
        words.append({
            "text": word_text,
            "phones": canon_phones,              # canonical (PYBZ)
            "phones_py": canon_phones_py,        # reference
            "phones_pybz": canon_phones,         # keep for backward compatibility
